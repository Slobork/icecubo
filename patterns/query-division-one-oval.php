<?php
// phpcs:ignore
if (! defined('ABSPATH') ) {
    exit;
}
/**
 * Title: Posts wrapped in a group. Oval post wrap.
 * Slug: icecubo/query-division-one-oval
 * Categories: icecubo-query-division-one
 */
?>
<!-- wp:group {"metadata":{"categories":["icecubo-query-division-one"],"patternName":"icecubo/query-division-one-oval","name":"Posts wrapped in a group. Oval post wrap."},"style":{"spacing":{"margin":{"top":"var:preset|spacing|xxx-large"}}},"layout":{"type":"default"}} -->
<div class="wp-block-group" style="margin-top:var(--wp--preset--spacing--xxx-large)"><!-- wp:group {"align":"wide","style":{"spacing":{"margin":{"bottom":"var:preset|spacing|large"},"padding":{"top":"0px","bottom":"0px"}},"border":{"radius":"40px"}},"backgroundColor":"white-ice","layout":{"type":"default"}} -->
<div class="wp-block-group alignwide has-white-ice-background-color has-background" style="border-radius:40px;margin-bottom:var(--wp--preset--spacing--large);padding-top:0px;padding-bottom:0px"><!-- wp:heading {"className":"is-style-icecubo-underline-overline"} -->
<h2 class="wp-block-heading is-style-icecubo-underline-overline">TECHNOLOGY</h2>
<!-- /wp:heading --></div>
<!-- /wp:group -->

<!-- wp:query {"queryId":0,"query":{"pages":0,"offset":0,"postType":"post","order":"desc","orderBy":"date","author":"","search":"","exclude":[],"sticky":"exclude","type":"constrained","inherit":false,"perPage":"3"},"align":"wide","layout":{"type":"default"}} -->
<div class="wp-block-query alignwide"><!-- wp:post-template {"layout":{"type":"grid","columnCount":3}} -->
<!-- wp:group {"className":"is-style-default","style":{"spacing":{"margin":{"bottom":"var:preset|spacing|xx-large"}},"border":{"radius":"40px"},"elements":{"link":{"color":{"text":"var:preset|color|primary"}}}},"backgroundColor":"white-ice","textColor":"darko-down-bottom"} -->
<div class="wp-block-group is-style-default has-darko-down-bottom-color has-white-ice-background-color has-text-color has-background has-link-color" style="border-radius:40px;margin-bottom:var(--wp--preset--spacing--xx-large)"><!-- wp:post-title {"isLink":true,"style":{"spacing":{"margin":{"top":"var:preset|spacing|medium"}}},"fontSize":"large"} /-->

<!-- wp:post-featured-image {"isLink":true,"height":"130px","style":{"border":{"width":"4px"}}} /-->

<!-- wp:group {"style":{"spacing":{"blockGap":".25rem"}},"fontSize":"extra-small","layout":{"type":"flex"}} -->
<div class="wp-block-group has-extra-small-font-size"><!-- wp:post-date {"className":"is-style-default"} /-->

<!-- wp:paragraph -->
<p>·</p>
<!-- /wp:paragraph -->

<!-- wp:post-author {"showAvatar":false,"className":"is-style-default"} /--></div>
<!-- /wp:group -->

<!-- wp:post-excerpt {"showMoreOnNewLine":false,"excerptLength":20} /-->

<!-- wp:group {"className":"has-extra-small-font-size","style":{"spacing":{"blockGap":".5rem"},"fontSize":"extra-small"},"layout":{"type":"flex"}} -->
<div class="wp-block-group has-extra-small-font-size"><!-- wp:post-terms {"term":"category","className":"is-style-default"} /-->

<!-- wp:post-terms {"term":"post_tag","className":"is-style-default"} /--></div>
<!-- /wp:group --></div>
<!-- /wp:group -->
<!-- /wp:post-template --></div>
<!-- /wp:query -->

<!-- wp:paragraph {"align":"right"} -->
<p class="has-text-align-right">→See more from <a href="#">TECHNOLOGY</a></p>
<!-- /wp:paragraph --></div>
<!-- /wp:group -->